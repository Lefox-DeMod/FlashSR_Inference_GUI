...
class FlashSR(DDPM):
    def __init__(
        ...
        # Check if CUDA avalaible for selection 'map_location' or using CPU only
        map_location = torch.device('cpu') if not torch.cuda.is_available() else None
        print(f"Loading model weights on {'CPU' if map_location else 'GPU'}")

        # Loading weights based on map_location
        student_ldm_state_dict = torch.load(student_ldm_ckpt_path, map_location=map_location)
        self.load_state_dict(student_ldm_state_dict)

        self.vae = VAEWrapper(autoencoder_ckpt_path)

        self.sr_vocoder = SRVocoder()
        sr_vocoder_state_dict = torch.load(sr_vocoder_ckpt_path, map_location=map_location)
